				...
				<section class="postlist">
					...
					<div class="querybar">
						...
						<ul class="dripper">
							...
							<li>
								<a href="#" class="filter" data-filter="all"><?php the_title(); ?></a>
								<ul class="drip">
						...
						<div class="post count<?php echo $postcount; ?><?php if(!$imgsrc) { ?> noimage<?php } ?>"<?php if($wp_query->found_posts == 1) { ?> style="width: 100%"<?php } ?>>
							...
							<div class="dearfriend">
								...
								<div class="p-lining">
									...
									<h4 class="cats"><?php _e('In','misfitlang'); ?> <?php $project_terms = wp_get_object_terms($post->ID, 'category'); if(!empty($project_terms)) { if(!is_wp_error( $project_terms )) { echo ''; $count = 0; foreach($project_terms as $term){ if($count > 0) { echo ', '; } echo '<a href="'.get_term_link($term->slug, 'category'). '">'.$term->name. '</a>';  $count++; }  } } ?></h4>
